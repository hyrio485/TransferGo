...
安装 TransferGo 和 ffmpeg 后，只需编码文件、用手机拍摄视频，再解码手机拍摄的视频。

### 1．编码文件

```bash
./transfergo encode input.bin
```
...
在电脑上全屏播放 `input.bin.mp4`，再用手机相机完整拍摄播放过程。拍摄时请保持画面稳定，确保所有二维码完整、清晰地出现在画面中，并从第一个画面录制到最后一个画面。

拍摄完成后，把手机视频复制回电脑。下面假设视频文件名为 `recorded.mp4`。

### 3．解码手机视频

```bash
./transfergo decode recorded.mp4
```

解码成功后，程序会使用视频清单中记录的原文件名输出文件，本例中为 `input.bin`。如果当前目录已经存在同名文件，可以通过 `-o restored.bin` 指定其他输出路径。
...
- 默认以 `175` 像素的二维码尺寸排列 `3×3` 个二维码，并自动计算视频长宽。
- 支持自定义视频帧率、二维码尺寸、网格行列数、数据块大小和 H.264 CRF。
- 默认使用多个工作协程并行生成和解码 PNG 图片。
...
5. 把协议帧编码为二维码，并按 `-rows × -cols` 分组生成 PNG 图片；默认并行处理不同图片。
...
`encode` 和 `decode` 都接受一个位置参数作为输入路径。
...
| 参数 | 默认值 | 说明 |
| --- | --- | --- |
| `-i`、`-in` | 位置参数中的文件 | 输入文件，可代替位置参数。 |
| `-o`、`-out` | 原文件名后追加 `.mp4` | 输出视频路径。 |
| `-p`、`-password` | 空 | AES-GCM 加密密码；非空时启用加密。 |
| `-ffmpeg` | 自动查找 | ffmpeg 可执行文件路径。 |
| `-frames-dir` | 随机临时目录 | 生成的 PNG 帧目录。 |
| `-fps` | `3` | 输出视频帧率，必须是大于零的有限数值。 |
| `-qr-size` | `175` | 单个二维码的宽高像素数，必须大于零。 |
| `-qr-error-correction` | `L` | 二维码纠错等级，可选 `L`、`M`、`Q`、`H`；等级越高，容错能力越强、可承载数据越少。 |
| `-width` | 自动计算 | 输出视频宽度；指定时必须是正偶数。 |
| `-height` | 自动计算 | 输出视频高度；指定时必须是正偶数。 |
...
| `-parallel` | `true` | 是否使用多个工作协程并行生成 PNG 图片。 |
...
生成的视频中，二维码区域的背景色为白色，视频背景色为淡灰色；为获得最佳识别效果，请将二维码的白色背景区域调整得尽量小。

推荐的视频尺寸与二维码大小的关系是：`长度／宽度＝二维码个数×二维码大小＋（二维码个数＋1）×二维码大小／5`。

未指定 `-width` 或 `-height` 时，TransferGo 会按上述关系分别根据列数或行数自动计算；如果结果为奇数，会向上补齐为偶数以满足 `yuv420p` 的编码要求。显式指定的尺寸不会被自动调整。

默认使用多个工作协程并行生成 PNG 图片，工作协程数不会超过逻辑处理器数量或待生成图片数量。需要降低瞬时 CPU 和内存占用时，可以传入 `-parallel=false` 切换为串行生成。ffmpeg 是否使用内部多线程由其编码器自行决定，不受该参数控制。
...
| `-rows` | 自动读取清单 | 手动指定每张图片的二维码行数，必须与 `-cols` 同时使用。 |
| `-cols` | 自动读取清单 | 手动指定每张图片的二维码列数，必须与 `-rows` 同时使用。 |
...
默认从文件清单读取二维码网格。清单二维码难以识别或旧版清单不包含网格时，可以同时指定 `-rows` 和 `-cols`；手动参数优先于清单记录。
